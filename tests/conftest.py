...
def my_cube(nv=N_VELOCITY_BINS, nx1=NX, nx2=NY, vlim=VELOCITY_LIMIT):
    ...
    cube = pkm.ifu_cube.IFUCube(ssps=ssps, nx1=nx1, nx2=nx2, nv=nv, vrng=(-vlim, vlim))
    return cube
...
def my_component(eval_ybar=EVAL_YBAR, cube=my_cube()):
    gc1 = pkm.components.GrowingDisk(cube=cube, rotation=0.0, center=(0.2, 0))
    gc1.set_p_t(lmd=2.0, phi=0.8)
    gc1.set_p_x_t(q_lims=(0.05, 0.5), rc_lims=(1.0, 1.0), alpha_lims=(1.2, 0.8))
    gc1.set_t_dep(q=0.2, alpha=3.0, t_dep_in=0.5, t_dep_out=5.0)
    gc1.set_p_z_tx()
    gc1.set_mu_v(q_lims=(0.5, 0.1), rmax_lims=(0.5, 1.1), vmax_lims=(250.0, 100.0))
    gc1.set_sig_v(
        q_lims=(0.25, 1.0),
        alpha_lims=(3.0, 2.5),
        sig_v_in_lims=(70.0, 50.0),
        sig_v_out_lims=(80.0, 60.0),
    )
    if eval_ybar:
        gc1.evaluate_ybar()
    return gc1
...
def my_second_component(eval_ybar=EVAL_YBAR, cube=my_cube()):
    gc2 = pkm.components.GrowingDisk(
        cube=cube, rotation=np.deg2rad(10.0), center=(0.05, -0.07)
    )
    gc2.set_p_t(lmd=1.6, phi=0.3)
    gc2.set_p_x_t(q_lims=(1.0, 1.0), rc_lims=(1.0, 1.0), alpha_lims=(1.1, 1.1))
    gc2.set_t_dep(q=0.7, alpha=1.1, t_dep_in=6.0, t_dep_out=1.0)
    gc2.set_p_z_tx()
    gc2.set_mu_v(q_lims=(0.5, 0.4), rmax_lims=(0.7, 0.2), vmax_lims=(-150.0, -190.0))
    gc2.set_sig_v(
        q_lims=(0.3 / 0.4, 0.4 / 0.3),
        alpha_lims=(2.0, 1.5),
        sig_v_in_lims=(70.0, 90.0),
        sig_v_out_lims=(10.0, 20.0),
    )
    if eval_ybar:
        gc2.evaluate_ybar()
    return gc2
...
def my_stream_component(eval_ybar=EVAL_YBAR, cube=my_cube()):
    stream = pkm.components.Stream(cube=cube, rotation=0.0, center=(0.0, 0))
    stream.set_p_t(lmd=15.0, phi=0.3)
    stream.set_p_x_t(
        theta_lims=[-np.pi / 2.0, 0.75 * np.pi],
        mu_r_lims=[0.2, 0.8],
        nsmp=1000,
        sig=0.1,
    )
    stream.set_t_dep(t_dep=5.0)
    stream.set_p_z_tx()
    stream.set_mu_v(mu_v_lims=[-80, 100])
    stream.set_sig_v(sig_v=110.0)
    if eval_ybar:
...
def my_galaxy(eval_ybar=EVAL_YBAR, cube=my_cube()):
    ...
    galaxy = pkm.components.Mixture(
        cube=cube, component_list=[gc1, gc2, stream], weights=[0.65, 0.25, 0.1]
    )
    if eval_ybar:
...
def my_base_component(cube, galaxy, eval_ybar=EVAL_YBAR):
    log_p_tvxz = galaxy.get_log_p(
        "tvxz", density=True, light_weighted=False, collapse_cmps=True
    )
