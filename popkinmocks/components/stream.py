...
class Stream(parametric.ParametricComponent):
    ...
    def set_p_x_t(
        self, theta_lims=[0.0, np.pi / 2.0], mu_r_lims=[0.7, 0.1], sig=0.03, nsmp=75
    ):
        ...
        self.p_x_pars = dict(
            theta_lims=theta_lims, mu_r_lims=mu_r_lims, sig=sig, nsmp=nsmp
        )
        nt = len(self.cube.get_variable_values("t"))
        tmp = np.array(
            [
                self.xxp[:, :, np.newaxis] + cube.dx / 2.0,
                self.xxp[:, :, np.newaxis] - cube.dx / 2.0,
            ]
        )
        tmp = nrm_x.logcdf(tmp)
        log_p_x = special.logsumexp(tmp.T, -1, b=[1.0, -1.0]).T
        tmp = np.array(
            [
                self.yyp[:, :, np.newaxis] + cube.dy / 2.0,
                self.yyp[:, :, np.newaxis] - cube.dy / 2.0,
            ]
        )
        tmp = nrm_y.logcdf(tmp)
        log_p_y = special.logsumexp(tmp.T, -1, b=[1.0, -1.0]).T
    ...
    def set_sig_v(self, sig_v=100.0):
